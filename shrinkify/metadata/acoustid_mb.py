...
class AcoustIDMetadata(object):
    def __init__(self, conf: config.Config, cache: caching.CacheConnector | None) -> None:
        ...
        if self.cache and self.rec_cache and self.rel_cache:
    ...
    def fetch(self, file: pathlib.Path):
        ...
        if self.cache and self.cache.contains(relativePath=str(file.relative_to(self.conf.general.root))):
            ...
            acoustid_resp = tuple(acoustid.match(self.conf.metadata.acoustid.api_key, str(file.expanduser())))
            if self.cache:
        ...
        for match in acoustid_resp:
            ...
            if self.rec_cache and self.rec_cache.contains(id=match[1]):
                ...
                time.sleep(1)
                rec_resp = self.session.get(f"https://musicbrainz.org/ws/2/recording/{match[1]}", params={'inc': 'releases'})
                time.sleep(1)
                ...
                if 'releases' not in recording: #strange off-case
                    continue
                if self.rec_cache:
                    self.rec_cache.insert({'id': match[1], 'data': self.cachify(recording)})
            ...
            for rec_release in recording['releases']:
                if self.rel_cache and self.rel_cache.contains(id=rec_release['id']):
                    ...
                    if ('cover-art-archive' not in release or not release['cover-art-archive']['front']) and not self.conf.metadata.acoustid.allow_missing_image:
                        continue
                    if self.rel_cache:
                        self.rel_cache.insert({'id': rec_release['id'], 'data': self.cachify(release)})
                    break
            else:
                continue
            break
        ...
        if 'date' in release: output['date'] = release['date']
        if 'date' in release: output['year'] = release['date'][0:4]
